...
public class CommonController {
    ...
    @ResponseBody
    public boolean checkLogin(String sname, String pass, HttpSession session) {
        log.info(sname);
        log.info(pass);

        //根据用户名找用户信息
        Student student = studentService.findByName(sname);
        if (student == null) {
    ...
    public boolean updatePwd(String old_pass, String new_pass, HttpSession session) {
        ...
        if (!student.getPass().equals(old_pass)) {
        ...
        studentService.updatePwd(sid, new_pass);
    ...
    public Map<String, Object> editorUploadImg(MultipartFile[] imgs) throws IOException {
        ...
        for (MultipartFile file : imgs) {
            ...
            OutputStream os = new FileOutputStream(CommonData.EXAM_IMAGES + filename);
